    ...
    fn solve(&self, method: Method, print_status: bool) -> Result<SolverOk, SolverErr> {
        ...
        match self.map {
            MapType::Goals(ref goals_map) => {
                ...
                match method {
                    Method::MoveOptimalMinPushes => Ok(solver.search(print_status, MoveLogic)),
            ...
            for &dir in &DIRECTIONS {
                ...
                if box_index < 255 {
                    ...
                    if box_grid[push_dest] == 255 && sd.closest_push_dists[push_dest].is_some() {
                        ...
                        let h = ComplexCost(push_dists_heuristic(sd, new_state), 0);
                        // cost is number of steps plus the push
                        new_states.push((&*new_state, ComplexCost(steps + 1, 0), h));
                    }
            ...
            for &dir in &DIRECTIONS {
                ...
                if box_index < 255 {
                    ...
                    if box_grid[push_dest] == 255 && sd.closest_push_dists[push_dest].is_some() {
                        ...
                        let h = SimpleCost(push_dists_heuristic(sd, &new_state));
                        new_states.push((&*new_state, SimpleCost(1), h));
                    }
                } else if sd.map.grid()[new_player_pos] != MapCell::Wall
                    && !reachable[new_player_pos]
                {
...
where
    M: Map + Clone, // TODO make Clone path of Map?
    Solver<M>: SolverTrait<M = M>,
{
    let mut new_states = Vec::new();
